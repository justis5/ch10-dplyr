...
data <- read.csv("data/nba_teams_2016.csv")
...
data <- mutate(data, tov_to_stl = TOV / STL)

# Sort the teams from lowest turnover/steal ratio to highest
# Which team has the lowest turnover/steal ratio?
data <- arrange(data, tov_to_stl)
lowest_tov_to_stl <- data %>% 
  filter(tov_to_stl == min(tov_to_stl)) %>% 
  select(Team)
...
better_shooters <- function(home, away) {
  data %>%
...
better_shooters("Boston Celtics*", "Houston Rockets*")
