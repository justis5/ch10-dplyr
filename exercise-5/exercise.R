...
arr_delay_by_month <- group_by(flights, dest) %>% 
  summarize(avg_delay = mean(arr_delay, na.rm = TRUE)) %>% 
  arrange(-avg_delay)
head(arr_delay_by_month)
# You can look up these airports in the `airports` data frame!
filter(airports, faa == "MSN")
filter(airports, faa == "TUL")

# Which city was flown to with the highest average speed?
city_highest_avg_sped <- flights %>% 
  mutate(speed = distance / air_time * 60) %>% 
  group_by(dest) %>% 
  summarize(avg_sped = mean(speed, na.rm = TRUE)) %>% 
  filter(avg_sped == max(avg_sped, na.rm = TRUE)) %>% 
  select(dest)
